...
M = size(f,1);
N = size(f,2);
noise = lognormal_noise(M,N,a,b);
max_value_n = max(max(noise));
min_value_n = min(min(noise));
noise = (noise-min_value_n)/(max_value_n-min_value_n);
fn = f + noise;
...
% improved filter
tol = 0.1;
fr_im = adaptive_mid_filter_improved(f,s_size,s_size_init,tol);
...
imshow(fr_im)
title('filtered photo by adaptive median with improvement')
...
% if z not within [z_min,z_max], it is probably salt & pepper
% so we should use z_med
fr = f;
for i = 1:size(f,1)
    for j = 1:size(f,2)
        % level A
...
function fr = adaptive_mid_filter_improved(f,s_size,s_size_init,tol)
...
for i = 1:size(f,1)
    for j = 1:size(f,2)
        ...
        if z_med_valid == true
            if within(z_out,z_min,z_max) == false
                if within(z_out,0,tol) || within(z_out,1-tol,1)
                    z_out = z_med;
                end
            end
        else
